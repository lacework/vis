...
class Label {
  ...
  addListeners(canvas) {
    ...
    let is_under_mouse = function(evt) {
      let linkX = self.size.left * self.body.view.scale + self.body.view.translation.x;
      let linkY = self.size.top  * self.body.view.scale + self.body.view.translation.y;
      let linkW = self.size.width;
      let linkH = self.size.height;
