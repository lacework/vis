...
class Label {
  ...
  addListeners(canvas) {
    ...
    let self = this;
    let is_under_mouse = function(evt) {
      let scale = self.body.view.scale;
      let linkX = self.size.left * scale + self.body.view.translation.x;
      let linkY = self.size.top  * scale + self.body.view.translation.y;
      let linkW = self.size.width * scale;
      let linkH = self.size.height * scale;
      let rect = canvas.getBoundingClientRect();

      let x = (evt.clientX - rect.left) //* scale; 
      let y = (evt.clientY - rect.top)  //* scale;

      //is the mouse over the link?
      if(x>=linkX && x <= (linkX + linkW) && y>=linkY && y<= (linkY+linkH)) {
        //console.info('box.x1: ' + linkX + ' | x: ' + x + ' | box.x2: ' + (linkX+linkW)
        //    + ' | box.y1: ' + linkY + ' | y: ' + y + ' | box.y2: ' + (linkY + linkH));
        return true;
      } else {
        return false;
      }
    ...
    let on_click = function(evt) {
      if(is_under_mouse(evt)) {
        self.body.emitter.emit('labelClick', self.nodeOptions);
      }
    }
