...
class InteractionHandler {
  ...
  canZoom(newScale) {
    ...
    if(this.body.nodeIndices.length <= 0)
      return false;
    ...
    if(this.options.zoomFactor !== undefined) {
      if(this.defaultScale === undefined) {
        ...
        this.maxScale = this.defaultScale * this.options.zoomFactor;
        this.minScale = this.defaultScale / this.options.zoomFactor; 
      }

      
      let zoomOut = true, oldScale = this.body.view.scale;
      if(oldScale < newScale) {
        zoomOut = false;
      }

      if((newScale >= this.maxScale) &&  (zoomOut === false)) 
        return false;

      if((newScale <= this.minScale) && (zoomOut === true))
        return false;
    }
