...
class InteractionHandler {
  ...
  onTap(event) {
    ...
    if(this.popup && this.popupHideTimer && this.popup.contains(pointer)) {
      ...
      if(_nodeOptions.onTooltipClick) {
        _nodeOptions.onTooltipClick(event, this.popupObj.id);
      }
      return;
    }
  ...
  onMouseMove(event) {
    ...
    */
    if (this.options.hover === true && this.popupTimer === undefined) {
      // adding hover highlights
 ...
 _checkShowPopup(pointer) {
  ...
  // hint for debug
  _hide() {
    if(this.popupHideTimer && this.popup) {
      return;
    } 
