...
class InteractionHandler {
  ...
  canZoom(newScale) {
    // no zoom if there are no nodes
    if(this.body.nodeIndices.length <= 0)
      return false;


    if(this.options.zoomFactor !== undefined) {
      let oldScale = this.body.view.scale;
      let zoomOut = true;
      let minZoom = 0, maxZoom = (this.options.zoomFactor * 2);

      if(this.defaultScale === undefined) {
        this.defaultScale = this.body.view.scale;
      } else if(newScale === this.defaultScale) {
        // reset currentZoom 
        this.currentZoom = this.options.zoomFactor;
      }
      ...
      if((this.currentZoom >= maxZoom) &&  (zoomOut === false)) 
        return false;

      if((this.currentZoom <= minZoom) && (zoomOut === true))
        return false;
    }
